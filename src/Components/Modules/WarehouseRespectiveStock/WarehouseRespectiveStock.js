...
const WarehouseStockItems = () => {
  ...
  const fetchStockData = async () => {
    try {
      ...
      if (response.data && response.data.result) {
        // Filter: Status must be 'Available' or 'Assigned', 
        // Stock_Point must NOT be 'MAIN STOCK ROOM'
        // AND Stock_Point must match the userName from localStorage
        const filtered = response.data.result.filter(
          (item) => {
            // Check if the item's Stock_Point matches the userName
            const matchesUser = userName && item.Stock_Point === userName.toUpperCase();
  ...
  // Show message if user is not logged in or no stock data
  if (!userName) {
    return (
      <div className="main-container">
        <div className="sales-table-container">
          <div className="text-center py-5">
            ...
            <p className="text-muted">Please log in to view your stock items.</p>
          </div>
        </div>
      </div>
    );
  ...
  // Show message if no items found for this user
      ...
      <div className="main-container">
        <div className="sales-table-container">
          <div className="text-center py-5">
            ...
            <p className="text-muted">No stock items found for {userName}.</p>
          </div>
        </div>
      </div>
    );
          ...
          <Col className="d-flex justify-content-between align-items-center">
            <div>
              ...
              <p className="text-muted mb-0" style={{ fontSize: '14px' }}>
                ...
                {userName && <Badge bg="info" className="ms-2">Filtered: {userName}</Badge>}
              </p>
            </div>
          </Col>
        </Row>
