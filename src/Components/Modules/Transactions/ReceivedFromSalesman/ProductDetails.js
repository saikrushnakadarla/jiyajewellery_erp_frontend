...
const ProductDetails = ({
    ...
    try {
      ...
      if (response.data.success && response.data.record) {
        ...
        if (isBagWeightCapture) {
          if (onReceivedCaptureWeightOfBagChange) {
            onReceivedCaptureWeightOfBagChange(record.total_grams);
          }
 ...
 const handleClear = () => {
  ...
  const doWeightsMatch = receivedCaptureWeightOfBag > 0 && assignedCaptureWeight > 0 &&
    Math.abs(receivedCaptureWeightOfBag - assignedCaptureWeight) < 0.001;
          ...
          {(packetTotals.grossWeight > 0 || packetTotals.packingWt > 0) && (
            <div style={{ marginTop: '10px' }}>
              <div style={{
                ...
                {/* NEW: Weight comparison UI for Received Salesman */}
                <div style={{ marginTop: '8px', display: 'flex', gap: '20px', alignItems: 'center', flexWrap: 'wrap' }}>
                  <div>
                    <span style={{ fontSize: '12px', color: '#666' }}>Capture Weight of AssignedSalesForm: </span>
                    <span style={{ fontSize: '14px', fontWeight: 'bold', color: '#a36e29' }}>
                      {assignedCaptureWeight ? assignedCaptureWeight.toFixed(3) : '0.000'}g
                    </span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <input
                      ...
                      step="0.001"
                      readOnly
                      value={receivedCaptureWeightOfBag || ''}
