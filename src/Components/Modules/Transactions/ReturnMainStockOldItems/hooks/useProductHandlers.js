...
import baseURL from './../../../../../Url/NodeBaseURL'; // ERP (port 5001)
import baseURL2 from './../../../../../Url/NodeBaseURL2'; // Jiya Jewellery (port 5000)
...
const useProductHandlers = (selectedSalesmanProducts = []) => {
  ...
  useEffect(() => {
    const fetchCurrentRates = async () => {
      try {
        ...
        const response = await axios.get(`${baseURL2}/get/current-rates`);
        const newRates = {
  ...
  // ===== FIXED: Use baseURL (ERP - port 5001) for opening-tags-entry =====
  ...
  const fetchSubCategory = async () => {
    try {
      // Use baseURL2 for Jiya Jewellery (port 5000)
      const response = await fetch(`${baseURL2}/subcategory`);
      const result = await response.json();
  ...
  useEffect(() => {
    const fetchMetalType = async () => {
      try {
        // Use baseURL2 for Jiya Jewellery (port 5000)
        const response = await fetch(`${baseURL2}/metaltype`);
  ...
  useEffect(() => {
    const fetchDesignName = async () => {
      try {
        // Use baseURL2 for Jiya Jewellery (port 5000)
        const response = await fetch(`${baseURL2}/designmaster`);
        if (!response.ok) {
          throw new Error("Failed to fetch products");
        }
        const result = await response.json();
  ...
  useEffect(() => {
    const fetchPurity = async () => {
      try {
        // Use baseURL2 for Jiya Jewellery (port 5000)
        const response = await fetch(`${baseURL2}/purity`);
        const data = await response.json();
  ...
  try {
    ...
    // ===== SECOND: Check if it's a tag from opening_tags_entry (data array) =====
    ...
    if (tag) {
      ...
      if (loggedInUserName && tag.Stock_Point !== loggedInUserName) {
        alert(`This product is not in your stock point (${tag.Stock_Point}). You can only transfer products from your stock point.`);
        setFormData((prevData) => ({
          ...prevData,
          code: "",
        }));
      ...
      setFormData((prevData) => ({
        ...
        tax_percent: productDetails?.tax_slab || tag.tax || "03% GST",
        qty: tag.pcs || 1,
        rate: rateValue || tag.rate || "",
        rate_amt: tag.rate_amt || "",
        total_price: tag.total_price || "",
        image: imagePath,
        imagePreview: imagePreview,
        cover_wt: tag.Cover_Wt || "",
        card_wt: tag.Card_Wt || "",
        packing_wt: tag.Packing_Wt || "",
        packet_barcode: tag.packetBarcode || null,
        is_estimated: !!tag.packetBarcode,
      }));
      setIsQtyEditable(false);
      return;
    }
    
    // ===== THIRD: Check if it's a product from products table =====
